...
## 7. TrajectoryRenderer — Camera path recording (19 septembre 2026)

### Contexte

Amélioration de la vue plan : afficher le chemin parcouru par la caméra sous forme d'une ligne blanche, pour mieux visualiser l'espace exploré lors du scanning et valider la couverture de l'environnement.

### Implémentation

Fichiers créés / modifiés :
- `core-render/src/main/kotlin/com/robot/render/TrajectoryRenderer.kt` — nouveau renderer
- `core-render/src/main/kotlin/com/robot/render/RenderConfig.kt` — constantes (8 lignes)
- `core-render/src/main/kotlin/com/robot/render/SlamRenderer.kt` — intégration (4 lignes)

### Design

**TrajectoryRenderer** enregistre la position world-space de la caméra et l'affiche en GL_LINE_STRIP :

- **Clé** : positions (x, y, z) depuis la matrice pose ARCore `c2w[12], c2w[13], c2w[14]`
- **Max points** : 8 000 (à 5 cm d'espacement → 400 m de chemin, suffisant pour une session indoor)
- **Filtre distance** : ne mémorise que si déplacement ≥ 5 cm en XZ (évite les points redondants si statique)
- **Upload GPU** : `DYNAMIC_DRAW` + `glBufferSubData()` incrémental — n'uploade que le nouveau point, pas le VBO complet
- **Rendu** : GL_LINE_STRIP, blanc (1.0, 1.0, 1.0, 0.8), actif dans la vue plan seulement
- **Thread-safety** : tous les appels sur GL thread (`init()` en `onSurfaceCreated()`, `addPoint()` en `onFrame()`, `draw()` en `drawFrame()`)

### Constantes (RenderConfig)

```kotlin
// ── Camera trajectory (plan view) ─────────────────────────────────────────
const val TRAJECTORY_MAX_POINTS  = 8_000
const val TRAJECTORY_MIN_DIST_M  = 0.05f
const val TRAJECTORY_COLOR_R     = 1.0f
const val TRAJECTORY_COLOR_G     = 1.0f
const val TRAJECTORY_COLOR_B     = 1.0f   // white line
const val TRAJECTORY_COLOR_A     = 0.8f
```

### Intégration SlamRenderer

```kotlin
// Champs
private val trajectoryRenderer = TrajectoryRenderer()

// onSurfaceCreated() → init GL resources
trajectoryRenderer.init()

// onFrame() → enregistre position caméra depuis c2w (après tracking pose)
trajectoryRenderer.addPoint(c2w[12], c2w[13], c2w[14])

// drawFrame() vue plan → affiche la trajectoire
trajectoryRenderer.draw(pv, pp)
```

### Validation

- ✅ TrajectoryRenderer compilé sans erreurs GL
- ✅ Constantes centralisées dans RenderConfig (no hardcode)
- ✅ Upload incrémental → pas de re-upload du VBO
- ✅ Affichage plan view uniquement (invisible en vue 3D camera)
...
| ~~Haute~~ | ~~Trajectoire caméra dans la vue PLAN~~ | **FAIT session 6** (TrajectoryRenderer + GL_LINE_STRIP) |
